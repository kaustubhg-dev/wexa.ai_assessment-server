...
async function computeCoPurchases(session) {
    await session.run(`
        MATCH (p1:Product)<-[:CONTAINS]-(:Order)<-[:PLACED]-(u:User)-[:PLACED]->(:Order)-[:CONTAINS]->(p2:Product)
        WHERE p1.type = p2.type AND p1 <> p2
        WITH p1, p2, count(DISTINCT u) AS coPurchasers
        MATCH (p1)<-[:CONTAINS]-(:Order)<-[:PLACED]-(buyersP1:User)
        WITH p1, p2, coPurchasers, count(DISTINCT buyersP1) AS totalP1
        WHERE totalP1 > 0
        MERGE (p1)-[r:CO_PURCHASED_WITH]->(p2)
        SET r.confidence = toFloat(coPurchasers) / totalP1, r.support = coPurchasers
    `);
    logger.info('Computed CO_PURCHASED_WITH edges');
}
